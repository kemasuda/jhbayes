...
if __name__ == "__main__":
    import pandas as pd
    name = "lina_m0.95-1.05"
    name = "lina_m0.90-1.10"
    #name = "lina_m0.90-1.10_pl"
    d = pd.read_csv(name+".csv")
    samples = np.load(name+".npz")['samples']

    #%%
    samples_1d = samples[:,:,1] # (n_systems, n_samples)

    #%%
    xbins = np.linspace(0, 14, 15*5)
    print (xbins)

    #%%
    hm = OnedHierarchical(samples_1d, xbins)
    ...
    #%%
    import dill
    with open(name+"_mcmc.pkl", "wb") as f:
        dill.dump(mcmc, f)

    #%%
    hm.summary_plots(mcmc, save=name, xlabel='age (Gyr)')

    #%%
    sall = dill.load(open("lina_m0.90-1.10_mcmc.pkl", 'rb')).get_samples()
    spl = dill.load(open("lina_m0.90-1.10_pl_mcmc.pkl", 'rb')).get_samples()

    #%%
    pratio = spl['priors'] / sall['priors']

    #%%
    qmean = np.median(pratio, axis=0)
    q5, q95 = np.percentile(pratio, [2.5,97.5], axis=0)

    #%%
    import matplotlib.pyplot as plt
    xarr, xidx = hm.xplot, hm.xidxplot
    #plt.xscale("log")
    plt.yscale("log")
    plt.axhline(y=1)
    plt.plot(xarr, qmean[xidx])
    plt.fill_between(xarr, q5[xidx], q95[xidx], alpha=0.2)
